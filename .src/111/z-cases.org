  ...
  namespace Nfit161
  {
      ...
      class TipCalc : Calculator
      {
          ...
          public override void ExportToBody(ISheet sheet)
          {
              if (sheet.LastRowNum == 0)
              {
                  ExcelHelper.CreateRowForSheet(sheet, 0, new[] {"账单", "费率", "小费", "应付"});
              }

              ExcelHelper.CreateRowForSheet(sheet, sheet.LastRowNum + 1,
                  new[] {Bill.ToString(), RATE.ToString(), Tip.ToString(), (Bill + Tip).ToString()});
          }
      }
      ...
      class ElectricBillCalc : Calculator
      {
          ...
          {
              if (sheet.LastRowNum == 0)
                  ExcelHelper.CreateRowForSheet(sheet, 0, new[] {"月", "量", "费用"});

              ExcelHelper.CreateRowForSheet(sheet, sheet.LastRowNum + 1,
                  new[] {_month.ToString(), _used.ToString(), Bill.ToString()});
          }
      }
  }
#+end_src
